...
namespace DSED_M07_TraitementCommande_Expedition
{
    internal class Subscriber
    {
        ...
        private const string nomFileMessage = "m07-courriel-premium";

        private const string sujetAbonnement = "commande.placee.premium";
            ...
            using (IConnection connexion = m_factory.CreateConnection())
            {
                using (IModel channel = connexion.CreateModel())
                {
                    ...
                    consommateur.Received += (model, ea) =>
                    {
                        ...
                        Console.WriteLine($"Commande premium -> {informationsCommande.Commande.Reference}");                     
